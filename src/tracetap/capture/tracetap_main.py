...
def run_proxy(listen_port=8080, raw_log_file=None, host_filter=None):
    """Run the proxy programmatically (called from CLI).
    ...
    if not addon_path.exists():
        raise RuntimeError(f"Cannot find tracetap_addon.py in {addon_path.parent}")
